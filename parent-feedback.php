      ...
      <div class="layout-container">
        ...
        <aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
          ...
          <ul class="menu-inner py-1">
            ...
            <li class="menu-item">
              <a href="parent-feedback.php" class="menu-link" id="active-side-menu">
        ...
        <div class="layout-page">
          ...
          <form action="parent-send-msg.php" method="POST">

          <div class="chat">                
                <span class="chatlabel" style="float: left;">From: &nbsp;<?php echo "{$info['fname']}" ?></span><br><br>
                

                <span>Sender Username: &nbsp;<input type="text" id="msginput" name="parent_id" value="<?php echo "{$info['id']}";?>"></span>
                <span class="teacher-select1">Recipient Username: &nbsp;</span>
                
                <span class="teacher-select2">
                  <select class="form-control" name="teacher_id" required placeholder="Select a recipient">
                      <option selected hidden></option>

<?php 

                      $sc_sql = "SELECT * FROM student_course WHERE student_id = '$student_id'";
                      ...
                      foreach($sc_info as $values0)
                      {
                        ...
                        #############################################
                        foreach($c_info as $values1)
                        {
                          $teacher_id = $values1[2];
                          $chat_sql = "SELECT * FROM teacher_table WHERE id = '$teacher_id'";
                          $chat_result = mysqli_query($data, $chat_sql);
                          $chat_info = mysqli_fetch_all($chat_result);
?>                      
<?php 
                          foreach($chat_info as $select)
                          {
                            $tname = $select[0];
                            $t_id = $select[2];                        
?>                            
                          <option value="<?php echo "$t_id"; ?>"> <?php echo "$tname - $c_name"; ?></option>                        
<?php 
                          }
                        }
                      }
?>                                        
              ...
              <div class="messagearea">
                <textarea class="form-control" id="messageto" name="message" placeholder="Type a message..." required></textarea> 
                <button type="submit" class="add" id="send">
                  ...
                  <span class="tooltiptext">Send message!</span>
                        ...
                        if($_SESSION['message'])
                          {
                            echo "<div class='alert alert-success alert-dismissible' role='alert' id='alert_toast' style='margin-top: 20px;'>"
                            .$_SESSION['message'].
                          ...
                          {
                            echo "<div class='alert alert-danger alert-dismissible' role='alert' id='alert_toast' style='margin-top: 20px;'>"
                            .$_SESSION['xmessage'].
                            "<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                          </div>";
                          }
              ...
              <h4 class="chat-head">Your messages</h4>

              <?php

                $parent_id = $info['id'];
                                          
                $chat_sql = "SELECT * FROM parent_feedback WHERE parent_id = '$id' ";
              ...
              <ul class="chat-container">
                ...
                <?php
                  ...
                  {                    
                    ...
                    foreach($t_info as $tname)
                    {                    
                      $fname = $tname[0];
                ?>

                    <li class="box3 sb13" id="right">
                      <span class="msglabel">To: <?php echo "{$tname[0]}" ?></span><br>
                      <p class="msg-main"><?php echo "{$chat[2]}" ?><p> 
                    </li>
