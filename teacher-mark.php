...
$courseid = $cid_info['id'];
$cl_id = $cid_info['class_id'];
            ...
            <div class="navbar-nav-right d-flex align-items-center" id="navbar-collapse">
              ...
              <div class="navbar-nav align-items-center">
                <div class="nav-item d-flex align-items-center">
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width=20 fill=grey>
                    <g data-name="Layer 2">
              ...
              <ul class="navbar-nav flex-row align-items-center ms-auto">
                ...
                <!--/ User -->
                      ...
                      <?php
                        if($_SESSION['message'])
                          {
                            echo "<div class='alert alert-success alert-dismissible' role='alert' id='alert_toast' style='margin: 20px auto; width: 75%; font-weight: bolder;'>"
                            .$_SESSION['message'].
                        ...
                        if($_SESSION['xmessage'])
                          {
                            echo "<div class='alert alert-danger alert-dismissible' role='alert' id='alert_toast' style='margin: 20px auto; width: 75%; font-weight: bolder;'>"
                            .$_SESSION['xmessage'].
                            "<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                          </div>";
                          }
            ...
            <div class="contain">
              ...
              <div class="card">
                ...
                <div class="table-responsive text-nowrap">
                  <table class="table table-hover">
                    <thead class="class">
                      <tr>
                        ...
                        <th class="class">Final Internal(%)</th>    
                      </tr>
                    </thead>
                    <tbody class="table-border-bottom-0">

                    <?php
                        $cs_sql = "SELECT * FROM student_course WHERE course_id = '$selected_course_id' ";
                        $cs_result = mysqli_query($data, $cs_sql);
                        $cs_info = mysqli_fetch_all($cs_result);
                        if (count($cs_info) > 0)
                        {
                          foreach($cs_info as $items)
                          {
                            $student_id = $items[0];
                            $class_id = $items[2];
                            $current_mark = $items[5];

                            $cln_sql = "SELECT * FROM class_admin WHERE class_id = '$class_id' ";
                            $cln_result = mysqli_query($data, $cln_sql);
                            $cln_info = mysqli_fetch_all($cln_result);
                            foreach($cln_info as $classname)
                            {
                              $class_name = $classname[4];
                             
                              if ($current_mark >= 80) {
                                  $borderColor = '#0ca50c4a';
                              } elseif ($current_mark >= 40 && $current_mark <= 79) {
                                  $borderColor = '#0000ff38';
                              } else {
                    ...
                    ?>
                      ...
                      <tr>
                        <td><?php echo "<a href='teacher-students.php?id={$student_id}'>$student_id</a>"; ?></td>
                        <td><?php echo "{$items[1]}"; ?></td>
                        <td><?php echo "{$items[4]}"; ?></td>
                        <td>
                          <input type="text" name="internal<?php echo "{$items[0]}";?>" value="<?php echo "$current_mark"; ?>" class="form-control internals" 
                          style="font-weight: bolder; color: #2f2f2f; background-color: <?php echo $borderColor; ?>;">
                        </td>
                  ...
                  <button type="submit" name="add-internal" class="btn btn-dark submit">Submit Internals</button>


                  <?php
                    if (isset($_POST['add-internal']))
                    {                    
                      foreach($cs_info as $items)
                      {
                        $std_id = $items[0];
                        ...
                        $int_mark="internal"."{$items[0]}";
                        $std_internal = $_POST[$int_mark];
                        $final_internal = doubleval($std_internal);

                        $internal_query = mysqli_query($data, "UPDATE student_course SET internal=$final_internal WHERE student_id = '$std_id' and course_id = '$crs_id' ");                      
                        if($internal_query)
                        {
                           $_SESSION['message'] = 'Internal marks submitted Succesfully!';
                           header("location:teacher-mark.php");
                        }
                        else
                        {
                            $_SESSION['xmessage'] = 'Failed add Internal marks. Try again!';
                            header("location:teacher-mark.php");
                        }
                      }
                    }
