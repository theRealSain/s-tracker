      ...
      <div class="layout-container">
        ...
        <aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
          ...
          <ul class="menu-inner py-1">
            ...
            <li class="menu-item">
              <a href="parent-attendance.php" class="menu-link" id="active-side-menu">
              <svg class="side-icons" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512" width=18 fill=#755dff>
            ...
            <li class="menu-item">
              <a href="parent-student-performance.php" class="menu-link">
        ...
        <div class="layout-page">
          ...
          <form action="" method="POST" id="attend-form">
            
          <section class="course-student">
            <form action="#" method="POST">
              <div class="studentview" style="margin: 40px 0px 80px 120px; width: 95%">
                <h2 class="c-h">
                <img src="favicon.ico">
                Student Attendance<h2>

                <select class="form-control" id="course-select" name="attendance_form" onchange="this.form.submit()">
                    ...
                    <?php
                      ...
                      // $c2_info = mysqli_fetch_assoc($c2_result);
                      // $course_name = $c2_info['cname'];
                    ...
                    ?> 
                      ...
                      <?php
                          if (isset($_POST['attendance_form'])) 
                          {
                            $selected_course_id=$_POST['attendance_form'];
                            $a_sql = "SELECT * FROM attendance_table WHERE student_id = '$student_id' AND course_id = '$selected_course_id';";
                            $total = mysqli_query($data,"SELECT COUNT(attendance) from attendance_table where student_id = '$student_id' and course_id = '$selected_course_id'");
                            $present = mysqli_query($data,"SELECT COUNT(attendance) from attendance_table where student_id = '$student_id' and course_id = '$selected_course_id' and attendance='Present';");                                            
                ...
                <span class="at-crs" id="cname">Course: <?php echo $cn_info['cname']; ?></span>

                <table class="c-add-table" id="show_attendance" style="width: 95%;">
                  <tr>
                    <th>Student ID</th>
                    <th>Date</th>
                    <th>Course</th>
                    <th>Status</th>
                        ...
                        <?php
                          $a_result = mysqli_query($data, $a_sql);
                          $a_info = mysqli_fetch_all($a_result);
                          foreach($a_info as $a_items)
                          {
                            $student_id = $a_items[0];
                            $attendance_date = $a_items[1];
                            $attendance = $a_items[2];
                            $course_id = $a_items[3];

                            $cn_sql = "SELECT * FROM courses WHERE id = '$course_id' ";
                            $cn_result = mysqli_query($data, $cn_sql);
                            $cn_info = mysqli_fetch_all($cn_result);

                            foreach($cn_info as $cn_items)
                            {
                              $cname = $cn_info[0];
                      ?>

                  <tr>
                    <td><?php echo $a_items[0]; ?></td>
                    <td><?php echo $a_items[1]; ?></td>                    
                    <td><?php echo $cn_items[0]; ?></td>
                    ...
                    <?php
                      ...
                      {
                        $class = "status-default"; // You can define a default class if needed
                      }
                    ?>

                    <td><span class="status <?php echo "$class"; ?>"><?php echo $a_items[2]; ?></span></td>
                      <?php
                            }
                          }
                        }
                ...
                <span class="at-perc" id="percent">Attendance: 
                  <?php  
                      ...
                      if (isset($_POST['attendance_form'])) 
                      {
                        ...
                        $total = mysqli_query($data,"SELECT COUNT(attendance) from attendance_table where student_id = '$student_id' and course_id = '$selected_course_id'");
                        $present = mysqli_query($data,"SELECT COUNT(attendance) from attendance_table where student_id = '$student_id' and course_id = '$selected_course_id' and attendance='Present';");                                          
                        $present_result=mysqli_fetch_array($present);
                        $present_result_value=$present_result[0];
                        if($present_result_value>0)
                        {                                                  
                          $total_result = mysqli_fetch_array($total);
                          $total_result_value=$total_result[0];
                          $percentage=($present_result_value/$total_result_value)*100;
                        ...
                        else
                        {
                          ...
                          echo "<script>document.getElementById('percent').innerHTML='No records found!';</script>";
                        }
                      }
                      ?>
                ...
                <script>
                  document.getElementById('attend-form').addEventListener('submit', cDisplay);
                  function cDisplay()
                  {
