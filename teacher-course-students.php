        ...
        <div class="layout-page">
          ...
          <form action="" method="POST" id="course_form">
            <div class="contain">
                  ...
                  <?php              
                    $cl_sql = "SELECT * FROM class_admin WHERE class_id = '$cl_id'";
                    $cl_result = mysqli_query($data, $cl_sql);
                    $cl_info = mysqli_fetch_assoc($cl_result);
                    $cl_name = $cl_info['class_name'];
                  ?>
                  
                <h1 class="contain-head"><img src="Images/favicon.ico">students of <?php echo $cname; ?></h1>

                <p class="students-intro">
                  This section displays all the students of <span class="highlight"><?php echo $cname; ?></span> of class
                  <span class="highlight"><?php echo $cl_name; ?></span>. <br>
                </p>

              <div class="card">
                <h5 class="card-header" id="attend-table-head">Students of (<?php echo "$cname"; ?>)</h5>                
                <div class="table-responsive text-nowrap">
                  <table class="table table-hover">
                    ...
                    <?php
                        $tc_sql = "SELECT * FROM student_course WHERE course_id = '$course_id' ";
                        $tc_result = mysqli_query($data, $tc_sql);
                        $tc_info = mysqli_fetch_all($tc_result);
                        if (count($tc_info) > 0)
                        {
                          foreach($tc_info as $items)
                          {
                            $student_id = $items[0];
                            $class_id = $items[2];
