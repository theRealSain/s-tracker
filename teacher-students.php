      ...
      <div class="layout-container">
        ...
        <aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
          ...
          <ul class="menu-inner py-1">
            ...
            <li class="menu-item">
              <a href="teacher-courses.php" class="menu-link" id="active-side-menu">
              <svg class="active-icon"  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" width=16 fill=white>
        ...
        <div class="layout-page">
          ...
          >
            ...
            <?php
                $get_std_id = $_GET['id'];
                $std_sql = "SELECT * FROM student_table WHERE id = '$get_std_id'";
                $std_result = mysqli_query($data, $std_sql);
                $std_info = mysqli_fetch_assoc($std_result);

                $sname = $std_info['fname'];
                $std_id = $std_info['id'];
                $sname = $std_info['fname'];
                $gender = $std_info['gender'];
                $dob = $std_info['dob'];
                $phone = $std_info['phone'];
                $class_id = $std_info['class_id'];
                $clg_id = $std_info['college_id'];
                
                $clname_sql = "SELECT * FROM class_admin WHERE class_id = '$class_id'";
                $clname_result = mysqli_query($data, $clname_sql);
                $clname_info = mysqli_fetch_assoc($clname_result);
                $clname = $clname_info['class_name'];

                $clg_sql = "SELECT * FROM admin_table WHERE college_id = '$clg_id'";
                $clg_result = mysqli_query($data, $clg_sql);
                $clg_info = mysqli_fetch_assoc($clg_result);
                $clg = $clg_info['college_name'];
            ?>
          ...
          <form action="" method="POST" id="course_form">
            <div class="contain" style="">
                ...
                <div class="sub-profile-grid">
                    ...
                    <div class="sub-profile-card">
                      <?php
                        $sql = "SELECT profile_picture FROM student_table WHERE id = '$get_std_id'";
                        $result = $data->query($sql);
                        if ($result->num_rows > 0)
                        {
                          $row = $result->fetch_assoc();
                    ...
                    <div class="sub-profile-card sub-details">
                      <div class="sub-details-1">
                        ...
                        <span class="sub-profile-label2"><?php echo "$std_id";?> </span>
                        ...
                        <span class="sub-profile-label2"><?php echo "$clg";?></span>
                      ...
                      <span class="sub-profile-label1">Gender</span>
                        <span class="sub-profile-label2"><?php echo "$gender";?>   </span>

                        <span class="sub-profile-label1">Date of Birth</span>
                        <span class="sub-profile-label2"><?php echo "$dob";?>  </span>

                        

                        <span class="sub-profile-label1">Phone</span>
                        <span class="sub-profile-label2"><?php echo "$phone";?></span>
                      </div>
                    </div>
            ...
            <div class="contain" style="margin-top: 30px">
                <h1 class="contain-head"><img src="Images/favicon.ico">student performance</h1>

                
                <div class="card">
                <h5 class="card-header"><b>Student performance based on various parameters.</b></h5>
                <div class="table-responsive text-nowrap">
                  <table class="table table-hover std-perf">
                    <thead>
                      <tr>

                        <?php

                          $s_sql = "SELECT * FROM student_course WHERE student_id = '$std_id'";
                          $s_result = mysqli_query($data, $s_sql);
                          $s_info = mysqli_fetch_assoc($s_result);
                          $course_name = $s_info['cname'];

                          $c_sql = "SELECT * FROM student_course WHERE student_id = '$std_id'";
                          $c_result = mysqli_query($data, $c_sql);
                        ...
                        <?php
                            ...
                            $cd_sql = "SELECT * FROM student_course WHERE cname = '$cname' AND student_id = $std_id";
                            $cd_result = mysqli_query($data, $cd_sql);
                            $cd_info = mysqli_fetch_all($cd_result);
                            echo "$cd_sql<br>";
                            foreach($cd_info as $crs_details)
                            {
                              $internal = $crs_details[5];
                              $t_rating = $crs_details[6];
                              $p_rating = $crs_details[7];
                              $attendance = $crs_details[8];

                              echo "<tr><td><b>Attendance</b></td> <td>$attendance</td></tr>
                                    <tr><td><b>Internal Marks</b></td> <td>$internal</td></tr>
                                    <tr><td><b>Teacher Evaluation</b></td> <td>$t_rating</td></tr>
                                    <tr><td><b>Parent Evaluation</b></td> <td>$p_rating</td></tr>";
                        ?>
