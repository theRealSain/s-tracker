...
$sql = "SELECT * FROM parent_table WHERE id = '$parent_id' ";
...
$sc_sql = "SELECT * FROM student_course WHERE student_id = '$student_id' AND course_id = '$course_id' ";

$sc_result = mysqli_query($data, $sc_sql);

$sc_info = mysqli_fetch_assoc($sc_result);

########################################

$tr_sql = "SELECT * FROM courses WHERE id = '$course_id' ";

$tr_result = mysqli_query($data, $tr_sql);

$tr_info = mysqli_fetch_assoc($tr_result);

$teacher_id = $tr_info['teacher_id'];

$t_sql = "SELECT * FROM teacher_table WHERE id = '$teacher_id' ";
...
$s_sql = "SELECT * FROM student_table WHERE id = '$student_id' ";

$s_result = mysqli_query($data, $s_sql);

$s_info = mysqli_fetch_assoc($s_result);

$std_name = $s_info['fname'];
...
>
  <head>
    ...
    <link rel="stylesheet" href="files-dashboard/profile.css" />
      ...
      <div class="layout-container">
        ...
        <aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
          ...
          <ul class="menu-inner py-1">
            ...
            <li class="menu-item">
              <a href="parent-courses.php" class="menu-link" id="active-side-menu">
              <svg class="side-icons"  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" width=18 fill=#755dff>
        ...
        <div class="layout-page">
          ...
          <form action="" method="POST">
            ...
            <section class="details-area">
              <h3 class="head"><img src="favicon.ico" width="35">student score prediction</h3>
              <span class="span1">Teacher: <?php echo "{$t_info['fname']}" ?></span>
              <span class="span2">Course: <?php echo "{$c_info['cname']}"; ?></span>
              

              <?php
$attendance = $sc_info['attendance'];
$internal = $sc_info['internal'];
$t_rating = $sc_info['t_rating'];
$p_rating = $sc_info['p_rating'];

// Execute the Python script and capture the output
$command = "python s-tracker.py $attendance $internal $t_rating $p_rating 2>&1";
$output = shell_exec($command);

if (!empty($output) && is_numeric($output)) {
    $prediction = floatval($output);
    $prediction = round($prediction, 2);
} else {
    echo "Error: Invalid prediction";
}
?>
              ...
              <table border=1 style="margin-bottom: 20px;">
                <tr>
                  <th>Name</th><td><?php echo "$std_name"; ?></td>
                </tr>
                <tr>
                  <th>Student ID</th><td><?php echo "{$info['student_id']}"; ?></td>
                </tr>
                <tr>
                  <th>Attendance</th><td><?php echo "$attendance%"; ?></td>
                </tr>
                <tr>
                  <th>Internal Marks</th><td><?php echo $internal; ?></td>
                </tr>
                <tr>
                  <th>Teacher Evaluation</th><td><?php echo $t_rating; ?></td>
                </tr>
                <tr>
                  <th>Parent Evaluation</th><td><?php echo $p_rating; ?></td>
                </tr>
              ...
              <span class="predict">The Predicted score for <?php echo "{$c_info['cname']}: $prediction%"; ?></span>

              <?php
              if ($prediction <= 35) 
               {
                 echo "<br>";
                 echo "<span style='color: red;text-align: center; margin: auto;display: block; '>Score below passing threshold. Should work on improving performance!</span>";
               }
               if ($prediction >= 90) 
               {
                 echo "<br>";
                 echo "<span style='color: green;text-align: center; margin: auto;display: block; '>Excellent! The predicted score is exceptional!</span>";
               }
               ?>
