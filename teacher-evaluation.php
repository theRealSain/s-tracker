...
$t_sql = "SELECT * FROM courses WHERE teacher_id = '$id' ";
$t_result = mysqli_query($data, $t_sql);
$t_info = mysqli_fetch_assoc($t_result);

######################################

$course_id = $t_info['id'];
$cid_sql = "SELECT * FROM student_course WHERE course_id = '$course_id' ";
$cid_result = mysqli_query($data, $cid_sql);
      ...
      <div class="layout-container">
        ...
        <aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
          ...
          <ul class="menu-inner py-1">
            ...
            <li class="menu-item">
              <a href="teacher-eval-intro.php" class="menu-link" id="active-side-menu">
              <svg class="active-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" width=18 fill=white>
              ...
              <ul class="navbar-nav flex-row align-items-center ms-auto">
                ...
                <!--/ User -->
                      ...
                      <?php
                        if($_SESSION['message'])
                          {
                            echo "<div class='alert alert-success alert-dismissible reaction-popup' role='alert' id='alert_toast'>"
                            .$_SESSION['message'].
                        ...
                        if($_SESSION['xmessage'])
                          {
                            echo "<div class='alert alert-danger alert-dismissible reaction-popup' role='alert' id='alert_toast' style='margin-top: 20px;'>"
                            .$_SESSION['xmessage'].
                            "<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                          </div>";
                          }
        ...
        $classid = $cname_info['class_id'];
                                
                                
                                
        $cl_sql = "SELECT * FROM class_admin WHERE class_id = '$classid'";
        $cl_result = mysqli_query($data, $cl_sql);
        $cl_info = mysqli_fetch_assoc($cl_result);
        $cl_name = $cl_info['class_name'];
        ?>
        ...
        <section class="class-teachers">
          <form action="" method="POST" id="course_form">
            <div class="contain">                  
                <h1 class="contain-head"><img src="Images/favicon.ico">students of <?php echo $course_name; ?></h1>

                <p class="students-intro">
                  This section displays all the students of <span class="highlight"><?php echo $course_name; ?></span> of class
                  <span class="highlight"><?php echo "$cl_name"; ?></span>. <br>
                </p>

              <div class="card">
                <h5 class="card-header" id="attend-table-head">Students of (<?php echo "$course_name"; ?>)</h5>                
                <div class="table-responsive text-nowrap">
                  <table class="table table-hover">
                    <thead class="class">
                      <tr>
                        ...
                        <th class="class">Evaluation(%)</th>    
                      </tr>
                    </thead>
                    <tbody class="table-border-bottom-0">

                    <?php
                      $ev_sql = "SELECT * FROM student_course WHERE course_id = '$selected_course_id'";  
                      $ev_result = mysqli_query($data, $ev_sql);
                      $ev_info = mysqli_fetch_all($ev_result);
                      if (count($ev_info) > 0)
                      {
                        foreach($ev_info as $eval)
                        {
                          $st_id = $eval[0];
                          $sname = $eval[1];
                          $cname = $eval[4];
                    ...
                    ?>
                      ...
                      <tr>
                        <td><?php echo "<a href='teacher-students.php?id={$st_id}'>$st_id</a>"; ?></td>
                        <td><?php echo "$sname"; ?></td>
                        <td><?php echo "$cname"; ?></td>
                        <td>
                          <input type="range" class="slider" name="slider<?php echo "$st_id";?>" <?php echo 'id="'; echo "$st_id"; echo '"'?> min=0 max=100 step=5 value=<?php echo "$t_rating"; ?>>
                          <span class="rating-label" id="t_rating<?php echo "$st_id";?>"></span>
                        </td>

                        <script>
                          var slider = document.getElementById("<?php echo "$st_id"; ?>");
                          var output = document.getElementById("<?php echo "t_rating"; echo"$st_id";?>");
                          output.innerHTML = slider.value;
                          slider.oninput = function()
                          {                        
                            document.getElementById("<?php echo "t_rating"; echo"$st_id";?>").innerHTML = this.value;
                            document.getElementById("<?php echo "t_rating"; echo"$st_id";?>").value = this.value;
                          }
                        </script>
                  ...
                  <button type="submit" name="add-rating" class="btn btn-dark submit">Submit Evaluation</button>
              ...
              <?php
                  if (isset($_POST['add-rating']))
                  {
                    foreach ($ev_info as $eval)
                    {
                      $std_id="{$eval[0]}";
                      $course_id = "{$eval[3]}";
                      $t_rating="slider"."{$eval[0]}";
                      
                      $std_t_rating=$_POST[$t_rating];
                      $std_t_rating=strval($std_t_rating);
                      $std_query=mysqli_query($data,"UPDATE student_course SET t_rating = '$std_t_rating' WHERE student_id = '$std_id' and course_id = '$course_id' ");                      
                      if($std_query)
                        {
                           $_SESSION['message'] = 'Rating submitted Succesfully!';
                           header("location:teacher-evaluation.php");
                        }
                        else
                        {
                            $_SESSION['xmessage'] = 'Failed add Rating. Try again!';
                            header("location:teacher-evaluation.php");
                        }
                    }
                  }
